...
$row_content_width = $content_text_aligment = $simple_background_color = $simple_background_image = $background_image_size = $disable_background_image = '';
...
if ( $row_content_width !== 'grid' ) {
	...
	if ( ! empty( $simple_background_image ) ) {
		if ( ! empty( $background_image_size ) ) {
			$eltdf_vc_row_style[] = 'background-size:' . esc_attr( $background_image_size );
		}
		$eltdf_vc_row_style[] = 'background-repeat: no-repeat';
	}
	...
	if ( ! empty( $simple_background_image ) ) {
		if ( ! empty( $background_image_size ) ) {
			$eltdf_grid_row_style[] = 'background-size:' . esc_attr( $background_image_size );
		}
		$eltdf_vc_row_style[] = 'background-repeat: no-repeat';
	}
...
if($row_content_width === 'grid'){
	$eltdf_row_wrapper_start .= '<div class="eltdf-row-grid-section-wrapper '.esc_attr($grid_row_classes).'" ' . frappe_elated_get_inline_style( $eltdf_grid_row_styles ) . '><div class="eltdf-row-grid-section">';
...
$output .= '<div ' . implode( ' ', $wrapper_attributes ) . ' ' . frappe_elated_get_inline_style( $eltdf_vc_row_styles ) . '>';
$output .= wpb_js_remove_wpautop( $content );
$output .= '</div>';
$output .= $eltdf_row_wrapper_end;
$output .= $after_output;

echo frappe_elated_get_module_part( $output );