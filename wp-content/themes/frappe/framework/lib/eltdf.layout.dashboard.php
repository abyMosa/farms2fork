...
/*
   Class: FrappeElatedClassDashboardForm
   A class that initializes Elated Dashboard Container
*/
class FrappeElatedClassDashboardForm implements iFrappeElatedInterfaceLayoutNode, iFrappeElatedInterfaceRender {
	public $children;
	public $name;
	public $form_id;
	public $form_method;
...
/*
   Class: FrappeElatedClassDashboardGroup
   A class that initializes FrappeElatedClass Group Field
*/
class FrappeElatedClassDashboardGroup implements iFrappeElatedInterfaceLayoutNode, iFrappeElatedInterfaceRender{
	public $children;
	public $name;
	public $title;
	public $description;
...
/*
   Class: FrappeElatedClassDashboardTitle
   A class that initializes Dashboard Title
*/
class FrappeElatedClassDashboardTitle implements iFrappeElatedInterfaceRender {
	private $name;
	private $title;
...
/*
   Class: FrappeElatedClassDashboardField
   A class that initializes FrappeElatedClass Front Field
*/

class FrappeElatedClassDashboardField implements iFrappeElatedInterfaceRender {
	private $type;
	private $name;
	private $label;
	private $description;
...
abstract class FrappeElatedClassDashboardFieldType {
	abstract public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = "" );
}

class FrappeElatedClassDashboardFieldText extends FrappeElatedClassDashboardFieldType {
...
class FrappeElatedClassDashboardFieldTextArea extends FrappeElatedClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class FrappeElatedClassDashboardFieldImage extends FrappeElatedClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        $class = '';
			...
			<div class="eltdf-dashboard-gallery-holder">
				<ul class="eltdf-dashboard-gallery-images-holder">
					<?php if ( isset($value_html) ) {
                        echo frappe_elated_get_module_part( $value_html );
					} ?>
				</ul>
				<div class="eltdf-dashboard-gallery-uploader">
					<?php 
					if ( frappe_membership_theme_installed() ) {
						echo frappe_elated_get_button_html( array(
							'text'      => esc_html__( 'Upload', 'frappe' ),
							'custom_class' => 'eltdf-dashboard-gallery-upload'
						) );
					} else {
						echo '<a itemprop="url" href="#" class="eltdf-btn eltdf-btn-medium eltdf-btn-solid eltdf-dashboard-gallery-upload"><span class="eltdf-btn-text">' . esc_html__( 'Upload', 'frappe' ) . '</span></a>';
					} ?>
					...
					<?php if ($value !== '' && $value !== false) { ?>
						<button class="eltdf-btn eltdf-btn-solid eltdf-dashboard-remove-image"><?php esc_html_e('Remove Media','frappe'); ?></button>
					<?php } ?>
				</div>
			</div>
		</div>
...
class FrappeElatedClassDashboardFieldGallery extends FrappeElatedClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        $class = '';
...
class FrappeElatedClassDashboardFieldSelect extends FrappeElatedClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        $class = '';
...
class FrappeElatedClassDashboardFieldDate extends FrappeElatedClassFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        if (!empty($repeat) && array_key_exists('name', $repeat) && array_key_exists('index', $repeat)) {
			$id		= $name . '-' . $repeat['index'];
			$name	= $repeat['name'] . '['.$repeat['index'].']['. $name .']';
...
class FrappeElatedClassDashboardFieldIcon extends FrappeElatedClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$options           = \FrappeElatedClassIconCollections::get_instance()->getIconCollectionsEmpty();
		$icons_collections = \FrappeElatedClassIconCollections::get_instance()->getIconCollectionsKeys();
				...
				<div class="eltdf-dashboard-item">
					...
					<div class="eltdf-dashboard-icon-holder">
						...
						<?php foreach ( $icons_collections as $icons_collection ) {
							$icons_param = \FrappeElatedClassIconCollections::get_instance()->getIconCollectionParamNameByKey( $icons_collection );
	                        $style       = 'display: none';
							...
							<div class="eltdf-icon-collection-holder" style="<?php echo esc_attr( $style ); ?>" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
                                <select name="<?php echo esc_attr( $name . '[' . $icons_param . ']'); ?>" id="<?php echo esc_attr( $name . '[' . $icons_param . ']'); ?>">
									<?php
									$icons = \FrappeElatedClassIconCollections::get_instance()->getIconCollection( $icons_collection );
...
class FrappeElatedClassDashboardFieldColor extends FrappeElatedClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        if (!empty($repeat) && array_key_exists('name', $repeat) && array_key_exists('index', $repeat)) {
			$id		= $name . '-' . $repeat['index'];
			$name	= $repeat['name'] . '['.$repeat['index'].']['. $name .']';
...
class FrappeElatedClassDashboardFieldCheckBoxGroup extends FrappeElatedClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class FrappeElatedClassDashboardFieldAddress extends FrappeElatedClassFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
		if ( isset( $args["col_width"] ) ) {
			$col_width = $args["col_width"];
			...
			<div class="eltdf-dashboard-field-row">
				<div class="eltdf-dashboard-item col-lg-<?php echo esc_attr($col_width); ?>">
					<div class="eltdf-dashboard-input-holder">
						...
						<a id="reset" href="#" style="display:none;"><?php esc_html_e( 'Reset Marker', 'frappe' ); ?></a>
					</div>
				</div>
			</div>
		</div>
...
class FrappeElatedClassDashboardRepeater implements iFrappeElatedInterfaceRender {
	private $label;
	private $description;
	private $name;
	private $fields;
	...
	function __construct( $fields, $name, $label = '', $description = '', $button_text = '', $table_layout = false, $value = array() ) {
		...
		$this->name        = $name;
		$this->num_of_rows = 1;
		$this->button_text = ! empty( $button_text ) ? $button_text : esc_html__( 'Add New Item', 'frappe' );
			...
			<?php if ($this->table_layout) { ?>
				<div class="eltdf-dashboard-repeater-table-heading">
					<div class="eltdf-dashboard-repeater-fields-holder">
						<div class="eltdf-dashboard-repeater-table-cell eltdf-dashboard-repeater-sort"><?php esc_html_e( 'Order', 'frappe' ) ?></div>
						...
						<div class="eltdf-dashboard-repeater-table-cell eltdf-dashboard-repeater-remove"><?php esc_html_e( 'Remove', 'frappe' ) ?></div>
					</div>
				</div>
			<?php } ?>
...
class FrappeElatedClassDashboardFieldFactory {
	public function render( $field_type, $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array()) {
		switch ( strtolower( $field_type ) ) {
			case 'text':
				$field = new FrappeElatedClassDashboardFieldText();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'textarea':
				$field = new FrappeElatedClassDashboardFieldTextArea();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'date':
				$field = new FrappeElatedClassDashboardFieldDate();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'image':
				$field = new FrappeElatedClassDashboardFieldImage();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'gallery':
				$field = new FrappeElatedClassDashboardFieldGallery();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'select':
				$field = new FrappeElatedClassDashboardFieldSelect();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'icon':
				$field = new FrappeElatedClassDashboardFieldIcon();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'color':
				$field = new FrappeElatedClassDashboardFieldColor();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'checkboxgroup':
				$field = new FrappeElatedClassDashboardFieldCheckBoxGroup();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'address':
				$field = new FrappeElatedClassDashboardFieldAddress();
