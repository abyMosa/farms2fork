...
/*
   Class: FrappeElatedClassFramework
...
class FrappeElatedClassFramework {
	...
	private function __construct() {
		$this->eltdOptions         = FrappeElatedClassOptions::get_instance();
		$this->eltdMetaBoxes       = FrappeElatedClassMetaBoxes::get_instance();
		$this->eltdTaxonomyOptions = FrappeElatedClassTaxonomyOptions::get_instance();
		$this->eltdUserOptions     = FrappeElatedClassUserOptions::get_instance();
		$this->eltdDashboardOptions = FrappeElatedClassDashboardOptions::get_instance();
	}
	...
	public function setSkin( FrappeElatedClassSkinAbstract $skinObject ) {
		$this->skin = $skinObject;
	}
}

/**
 * Class FrappeElatedClassSkinManager
 *
...
class FrappeElatedClassSkinManager {
	...
	/**
	 * @see FrappeElatedClassSkinManager::setSkin()
	 */
	...
	private function setSkin( $skinName = ELATED_PROFILE_SLUG ) {
		if ( $skinName !== '' ) {
			if ( file_exists( get_template_directory() . '/framework/admin/skins/' . $skinName . '/skin.php' ) ) {
				...
				$skinName = ucfirst( $skinName ) . esc_html__( 'Skin', 'frappe' );
...
/**
 * Class FrappeElatedClassSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class FrappeElatedClassSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	public function loadTemplatePart( $template, $params = array() ) {
		...
		if ( $template !== '' ) {
			include( frappe_elated_find_template_path( 'framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php' ) );
		}
	}
	...
	public function setShortcodeJSParams() { ?>
		<script>
			window.eltdSCIcon = '<?php echo frappe_elated_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
			window.eltdSCLabel = '<?php echo esc_html( ucfirst( $this->skinName ) ); ?> <?php esc_html_e( 'Shortcodes', 'frappe' ) ?>';
		</script>
	<?php }
...
/*
   Class: FrappeElatedClassOptions
...
class FrappeElatedClassOptions {
	...
	public function getOptionValue( $key ) {
		...
		if ( array_key_exists( $key, $frappe_elated_options ) ) {
			return $frappe_elated_options[ $key ];
...
/*
   Class: FrappeElatedClassAdminPage
   A class that initializes Elated Admin Page
*/
class FrappeElatedClassAdminPage implements iFrappeElatedInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $slug;
	public $title;
	...
	function __construct( $slug = "", $title_label = "", $icon = "" ) {
		$this->layout  = array();
		$this->factory = new FrappeElatedClassFieldFactory();
		$this->slug    = $slug;
	...
	public function renderChild( iFrappeElatedInterfaceRender $child ) {
		$child->render( $this->factory );
	}
}

/*
   Class: FrappeElatedClassMetaBoxes
...
/*
   Class: FrappeElatedClassMetaBox
   A class that initializes Elated Meta Box
*/
class FrappeElatedClassMetaBox implements iFrappeElatedInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	public $title;
	...
	public function setTitle( $label ) {
		...
		return $frappe_elated_Framework->getSkin()->getSkinLabel() . ' ' . $label;
	}
}

/*
   Class: FrappeElatedClassTaxonomyOptions
   A class that initializes FrappeElatedClass Taxonomy Options
*/
class FrappeElatedClassTaxonomyOptions {
	private static $instance;
...
/*
   Class: FrappeElatedClassTaxonomyOption
   A class that initializes FrappeElatedClass Taxonomy Option
*/
class FrappeElatedClassTaxonomyOption implements iFrappeElatedInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new FrappeElatedClassTaxonomyFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: FrappeElatedClassUserOptions
   A class that initializes FrappeElatedClass User Options
*/
class FrappeElatedClassUserOptions {
	private static $instance;
	public $userOptions;
...
/*
   Class: FrappeElatedClassUserOption
   A class that initializes FrappeElatedClass User Option
*/
class FrappeElatedClassUserOption implements iFrappeElatedInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new FrappeElatedClassUserFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: FrappeElatedClassDashboardOptions
   A class that initializes FrappeElatedClass Dashboard Options
*/
class FrappeElatedClassDashboardOptions {
	private static $instance;
...
/*
   Class: FrappeElatedClassDashboardOption
   A class that initializes FrappeElatedClass Dashboard Option
*/
class FrappeElatedClassDashboardOption implements iFrappeElatedInterfaceLayoutNode {
	public $layout;
	private $factory;
	
	function __construct() {
		$this->layout        = array();
		$this->factory       = new FrappeElatedClassDashboardFieldFactory();
	}
...
if ( ! function_exists( 'frappe_elated_init_framework_variable' ) ) {
	function frappe_elated_init_framework_variable() {
		global $frappe_elated_Framework;
		
		$frappe_elated_Framework = FrappeElatedClassFramework::get_instance();
		$eltdSkinManager                 = new FrappeElatedClassSkinManager();
		$frappe_elated_Framework->setSkin( $eltdSkinManager->getSkin() );
	}
	
	add_action( 'frappe_elated_action_before_options_map', 'frappe_elated_init_framework_variable' );
}
?>