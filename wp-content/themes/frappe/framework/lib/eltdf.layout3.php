...
/*
   Class: FrappeElatedClassMultipleImages
   A class that initializes Elated Multiple Images
*/

class FrappeElatedClassMultipleImages implements iFrappeElatedInterfaceRender {
	private $name;
	private $label;
	private $description;
	...
	function __construct( $name, $label = "", $description = "" ) {
		...
		$frappe_elated_Framework->eltdMetaBoxes->addOption( $this->name, "" );
	}
					...
					<div class="row">
						<div class="col-lg-12">
							...
							<div class="eltdf-gallery-uploader">
								<a class="eltdf-gallery-upload-btn btn btn-sm btn-primary" href="javascript:void(0)"><?php esc_html_e( 'Upload', 'frappe' ); ?></a>
								<a class="eltdf-gallery-clear-btn btn btn-sm btn-default pull-right" href="javascript:void(0)"><?php esc_html_e( 'Remove All', 'frappe' ); ?></a>
							</div>
						</div>
					</div>
				</div>
...
class FrappeElatedClassTwitterFramework implements iFrappeElatedInterfaceRender {
	public function render( $factory ) {
		$twitterApi = FrappeTwitterApi::getInstance();
		$message    = '';
		...
		if ( ! empty( $_GET['oauth_token'] ) && ! empty( $_GET['oauth_verifier'] ) ) {
			...
			if ( $responseObj->status ) {
				$message = esc_html__( 'You have successfully connected with your Twitter account. If you have any issues fetching data from Twitter try reconnecting.', 'frappe' );
			} else {
				$message = $responseObj->message;
			}
		}
		
		$buttonText = $twitterApi->hasUserConnected() ? esc_html__( 'Re-connect with Twitter', 'frappe' ) : esc_html__( 'Connect with Twitter', 'frappe' );
		?>
		<?php if ( $message !== '' ) { ?>
		...
		<div class="eltdf-page-form-section" id="eltdf_enable_social_share">
			<div class="eltdf-field-desc">
				<h4><?php esc_html_e( 'Connect with Twitter', 'frappe' ); ?></h4>
				<p><?php esc_html_e( 'Connecting with Twitter will enable you to show your latest tweets on your site', 'frappe' ); ?></p>
			</div>
			<div class="eltdf-section-content">
				<div class="container-fluid">
					<div class="row">
...
class FrappeElatedClassInstagramFramework implements iFrappeElatedInterfaceRender {
	public function render( $factory ) {
		$instagram_api = FrappeInstagramApi::getInstance();
		$message       = '';
		
		//if code wasn't saved to database
		if ( ! get_option( 'frappe_instagram_code' ) ) {
			...
			if ( ! empty( $_GET['code'] ) ) {
				...
				$message = esc_html__( 'You have successfully connected with your Instagram account. If you have any issues fetching data from Instagram try reconnecting.', 'frappe' );
		...
		$buttonText = $instagram_api->hasUserConnected() ? esc_html__( 'Re-connect with Instagram', 'frappe' ) : esc_html__( 'Connect with Instagram', 'frappe' );
		?>
		<?php if ( $message !== '' ) { ?>
			<div class="alert alert-success" style="margin-top: 20px;">
				<span><?php echo esc_html( $message ); ?></span>
		...
		<div class="eltdf-page-form-section" id="eltdf_enable_social_share">
			<div class="eltdf-field-desc">
				<h4><?php esc_html_e( 'Connect with Instagram', 'frappe' ); ?></h4>
				<p><?php esc_html_e( 'Connecting with Instagram will enable you to show your latest photos on your site', 'frappe' ); ?></p>
			</div>
			<div class="eltdf-section-content">
				<div class="container-fluid">
					<div class="row">
...
class FrappeElatedClassRepeater implements iFrappeElatedInterfaceRender {
	private $label;
	private $description;
	private $name;
	private $fields;
			...
			<?php if ($this->table_layout) { ?>
				<div class="eltdf-repeater-table-heading">
					<div class="eltdf-repeater-fields-holder">
						<div class="eltdf-repeater-table-cell eltdf-repeater-sort"><?php esc_html_e( 'Order', 'frappe' ) ?></div>
						...
						<div class="eltdf-repeater-table-cell eltdf-repeater-remove"><?php esc_html_e( 'Remove', 'frappe' ) ?></div>
					</div>
				</div>
			<?php } ?>
																			...
																			<div class="eltdf-repeater-inner-fields-row <?php echo esc_attr( $col_width_inner_class ); ?>">
																				<div class="eltdf-repeater-inner-fields-row-inner">
																					...
																					if ( ! empty( $field_inner['dependency'] ) ) {
																						$dependencyValues = frappe_elated_return_repeater_dependency_options_array(array(
																							'field' 	   => $field,
...
class FrappeElatedClassFieldAddress extends FrappeElatedClassFieldType {
				...
				<div class="container-fluid">
					<div class="row">
						...
						<?php endif; ?>
							...
							<button id="find" class="btn btn-primary"><?php esc_html_e( 'Place the pin on the map', 'frappe' ); ?></button>
...
class FrappeElatedClassFieldIcon extends FrappeElatedClassFieldType {
    public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $repeat = array() ) {
        $class = '';
        ...
        $icon_packs        = \FrappeElatedClassIconCollections::get_instance()->getIconCollectionsEmpty();
