...
class ElatedSkin extends FrappeElatedClassSkinAbstract {
    /**
     * Skin constructor. Hooks to frappe_elated_admin_scripts_init and frappe_elated_enqueue_admin_styles
     */
    public function __construct() {
        $this->skinName = 'elated';

        //hook to
        add_action('frappe_elated_action_admin_scripts_init', array($this, 'registerStyles'));
        add_action('frappe_elated_action_admin_scripts_init', array($this, 'registerScripts'));

        add_action('frappe_elated_action_enqueue_admin_styles', array($this, 'enqueueStyles'));
        add_action('frappe_elated_action_enqueue_admin_scripts', array($this, 'enqueueScripts'));

        add_action('frappe_elated_action_enqueue_meta_box_styles', array($this, 'enqueueStyles'));
        add_action('frappe_elated_action_enqueue_meta_box_scripts', array($this, 'enqueueScripts'));
	...
	public function registerScripts() {
		...
		foreach ( $this->scripts as $scriptHandle => $script ) {
			frappe_elated_register_skin_script( $scriptHandle, $script['path'], $script['dependency'] );
		}
	}
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
	        frappe_elated_register_skin_style($styleHandle, $stylePath);
        }
    }
    ...
    public function renderOptions() {
        ...
        $tab    = frappe_elated_get_admin_tab();
        $active_page = $frappe_elated_Framework->eltdOptions->getAdminPageFromSlug($tab);
     ...
     *
     * @see FrappeElatedClassSkinAbstract::loadTemplatePart()
     */
	 ...
	 *
	 * @param FrappeElatedClassAdminPage $page current page to load
	 * @see FrappeElatedClassSkinAbstract::loadTemplatePart()
	 */
