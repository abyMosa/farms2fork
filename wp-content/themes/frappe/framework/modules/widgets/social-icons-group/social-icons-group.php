...
class FrappeElatedClassClassIconsGroupWidget extends FrappeElatedClassWidget {
	public function __construct() {
		parent::__construct(
			'eltdf_social_icons_group_widget',
			esc_html__( 'Frappe Social Icons Group Widget', 'frappe' ),
			array( 'description' => esc_html__( 'Use this widget to add a group of up to 6 social icons to a widget area.', 'frappe' ) )
		);
	...
	protected function setParams() {
		$this->params = array_merge(
			...
			frappe_elated_icon_collections()->getSocialIconWidgetMultipleParamsArray(6),
			array(
				array(
					'type'    => 'dropdown',
					'name'    => 'layout',
					'title'   => esc_html__( 'Icons Layout', 'frappe' ),
					'options' => array(
						''             => esc_html__( 'Default', 'frappe' ),
						'square-icons' => esc_html__( 'Square', 'frappe' ),
					)
				),
				array(
					'type'        => 'dropdown',
					'name'        => 'skin',
					'title'       => esc_html__( 'Square Icons Skin', 'frappe' ),
					'description' => esc_html__( 'This applies to the square layout', 'frappe' ),
					'options'     => array(
						''           => esc_html__( 'Dark Skin', 'frappe' ),
						'light-skin' => esc_html__( 'Light Skin', 'frappe' ),
					)
				),
				array(
					'type'    => 'dropdown',
					'name'    => 'alignment',
					'title'   => esc_html__( 'Text Alignment', 'frappe' ),
					'options' => array(
				...
				array(
					'type'  => 'textfield',
					'name'  => 'icon_size',
					'title' => esc_html__( 'Icons Size (px)', 'frappe' )
				),
				array(
					'type'  => 'colorpicker',
					'name'  => 'color',
				...
				array(
					'type'  => 'colorpicker',
					'name'  => 'hover_color',
					'title' => esc_html__( 'Hover Color', 'frappe' )
				),
				array(
					'type'        => 'textfield',
					'name'        => 'margin',
					'title'       => esc_html__( 'Margin', 'frappe' ),
	...
	public function widget( $args, $instance ) {
		...
		if ( ! empty( $instance['icon_size'] ) ) {
			$icon_styles[] = 'font-size: ' . frappe_elated_filter_px( $instance['icon_size'] ) . 'px';
		}
		...
		for ( $n = 1; $n <= 6; $n++ ) {
			...
			<?php if ( ! empty( $icon_holder_html ) ) { ?>
				<a class="eltdf-social-icon-widget-holder eltdf-icon-has-hover" <?php echo frappe_elated_get_inline_attr( $hover_color, 'data-hover-color' ); ?> <?php frappe_elated_inline_style( $icon_styles ) ?>
