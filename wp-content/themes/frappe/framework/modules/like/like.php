...
class FrappeElatedClassLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_frappe_elated_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_frappe_elated_like', array( $this, 'ajax' ) );
	}
	...
	public function like_post( $post_id, $action = 'get', $type = '' ) {
		...
		switch ( $action ) {
			...
			case 'get':
				...
				$return_value = $icon . "<span>" . esc_attr( $like_count ) . esc_html__(' Likes', 'frappe')."</span>";
			...
			case 'update':
				...
				$return_value = "<i class='icon_heart' aria-hidden='true'> </i><span>" . esc_attr( $like_count ) . esc_html__(' Likes', 'frappe')."</span>";
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'frappe' );
		
		if ( isset( $_COOKIE[ 'eltdf-like_' . $post->ID ] ) ) {
			$class = 'eltdf-like liked';
			$title = esc_html__( 'You already like this!', 'frappe' );
		}
...
if ( ! function_exists( 'frappe_elated_create_like' ) ) {
	function frappe_elated_create_like() {
		FrappeElatedClassLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'frappe_elated_create_like' );
}