...
if ( ! function_exists( 'frappe_elated_get_blog_holder_params' ) ) {
	...
	function frappe_elated_get_blog_holder_params( $params ) {
		...
		$masonry_layout = frappe_elated_get_meta_field_intersect( 'blog_masonry_layout' );
...
if ( ! function_exists( 'frappe_elated_get_blog_list_classes' ) ) {
	/**
	 * Function that generates blog list holder classes for blog list templates
	 */
	function frappe_elated_get_blog_list_classes( $classes ) {
		$list_classes   = array();
		$list_classes[] = 'eltdf-blog-type-masonry';
		
		$number_of_columns = frappe_elated_get_meta_field_intersect( 'blog_masonry_number_of_columns' );
		if ( ! empty( $number_of_columns ) ) {
			$list_classes[] = 'eltdf-blog-' . $number_of_columns . '-columns';
		}
		
		$space_between_items = frappe_elated_get_meta_field_intersect( 'blog_masonry_space_between_items' );
	...
	add_filter( 'frappe_elated_filter_blog_list_classes', 'frappe_elated_get_blog_list_classes' );
}
