<?php
namespace FrappeCore\CPT\Shortcodes\ProductInfo;
...
class ProductInfo implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if(function_exists('vc_map')) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Product Info', 'frappe' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						array(
							...
							'heading'     => esc_html__( 'Selected Product', 'frappe' ),
							'settings'    => array(
								'sortable'      => true,
								'unique_values' => true
							),
							'admin_label' => true,
							'save_always' => true,
							'description' => esc_html__( 'If you left this field empty then product ID will be of the current page', 'frappe' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'product_info_type',
							'heading'     => esc_html__( 'Product Info Type', 'frappe' ),
							'value'       => array(
								esc_html__( 'Title', 'frappe' )          => 'title',
								esc_html__( 'Featured Image', 'frappe' ) => 'featured_image',
								esc_html__( 'Category', 'frappe' )       => 'category',
								esc_html__( 'Excerpt', 'frappe' )        => 'excerpt',
								esc_html__( 'Price', 'frappe' )          => 'price',
								esc_html__( 'Rating', 'frappe' )         => 'rating',
								esc_html__( 'Add To Cart', 'frappe' )    => 'add_to_cart',
								esc_html__( 'Tag', 'frappe' )            => 'tag',
								esc_html__( 'Author', 'frappe' )         => 'author',
								esc_html__( 'Date', 'frappe' )           => 'date',
							),
							'admin_label' => true
						),
						array(
							...
							'heading'    => esc_html__( 'Product Info Color', 'frappe' ),
						...
						array(
							...
							'value'       => array_flip( frappe_elated_get_title_tag( true, array( 'p' => 'p' ) ) ),
							'description' => esc_html__( 'Set title tag for product title element', 'frappe' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Featured Image Proportions', 'frappe' ),
							'value'      => array(
						...
						array(
							...
							'heading'     => esc_html__( 'Category Tag', 'frappe' ),
							'value'       => array_flip( frappe_elated_get_title_tag( true ) ),
							'description' => esc_html__( 'Set category tag for product category element', 'frappe' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Skin', 'frappe' ),
							'value'      => array(
								esc_html__( 'Default', 'frappe' ) => '',
								esc_html__( 'White', 'frappe' )   => 'white',
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Size', 'frappe' ),
							'value'      => array(
								esc_html__( 'Default', 'frappe' ) => '',
							...
							'dependency' => array( 'element' => 'product_info_type', 'value'   => array( 'frappe' ) ),
							'group'      => esc_html__( 'Product Info Style', 'frappe' )
						)
					)
				)
	...
	public function getItemTitleHtml( $params ) {
		...
		if ( ! empty( $title ) ) {
			$html = '<' . esc_attr( $title_tag ) . ' itemprop="name" class="eltdf-pi-title entry-title" ' . frappe_elated_get_inline_style( $product_info_style ) . '>';
			...
			$html .= '<div class="eltdf-pi-category">';
				foreach ( $categories as $cat ) {
					if ( ! empty( $params['category_tag'] ) ) {
						$html .= '<' . esc_attr( $params['category_tag'] ) . ' ' . frappe_elated_get_inline_style( $product_info_style ) . '>';
						...
						$html .= '<a itemprop="url" class="eltdf-pi-category-item" href="' . esc_url( get_term_link( $cat->term_id ) ) . '" ' . frappe_elated_get_inline_style( $product_info_style ) . '>' . esc_html( $cat->name ) . '</a>';
					}
				}
			$html .= '</div>';
		}
	...
	public function getItemExcerptHtml( $params ) {
		...
		if ( ! empty( $excerpt ) ) {
			$html = '<div class="eltdf-pi-excerpt"><p itemprop="description" class="eltdf-pi-excerpt-item" ' . frappe_elated_get_inline_style( $product_info_style ) . '>' . esc_html( $excerpt ) . '</p></div>';
		}
	...
	public function getItemPriceHtml( $params ) {
		...
		if ( $price_html = $product->get_price_html() ) {
			$html = '<div class="eltdf-pi-price" ' . frappe_elated_get_inline_style( $product_info_style ) . '>' . $price_html . '</div>';
		}
	...
	public function getItemRatingHtml( $params ) {
		...
		if ( get_option( 'woocommerce_enable_review_rating' ) !== 'no' ) {
			...
			$html = '<div class="eltdf-pi-rating" title="' . sprintf( esc_attr__( "Rated %s out of 5", "frappe" ), $average ) . '" ' . frappe_elated_get_inline_style( $product_info_style ) . '><span style="width:' . ( ( $average / 5 ) * 100 ) . '%"></span></div>';
		}
	...
	public function getItemAddToCartHtml( $params ) {
		...
		$html .= apply_filters( 'frappe_elated_filter_product_info_add_to_cart_link',
	...
	public function getItemTagHtml( $params ) {
		...
		if ( ! empty( $tags ) ) {
			$html = '<div class="eltdf-pi-tag">';
				foreach ( $tags as $tag ) {
					$html .= '<a itemprop="url" class="eltdf-pi-tag-item" href="' . esc_url( get_term_link( $tag->term_id ) ) . '" ' . frappe_elated_get_inline_style( $product_info_style ) . '>' . esc_html( $tag->name ) . '</a>';
				}
			$html .= '</div>';
		}
	...
	public function getItemAuthorHtml( $params ) {
		...
		if ( ! empty( $author ) ) {
			$html = '<div class="eltdf-pi-author" ' . frappe_elated_get_inline_style( $product_info_style ) . '>' . esc_html( $author ) . '</div>';
		}
	...
	public function getItemDateHtml( $params ) {
		...
		if ( ! empty( $date ) ) {
			$html = '<div class="eltdf-pi-date" ' . frappe_elated_get_inline_style( $product_info_style ) . '>' . esc_html( $date ) . '</div>';
		}
	...
	public function productIdAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				$data = array();
				$data['value'] = $value['id'];
				$data['label'] = esc_html__( 'Id', 'frappe' ) . ': ' . $value['id'] . ( ( strlen( $value['title'] ) > 0 ) ? ' - ' . esc_html__( 'Title', 'frappe' ) . ': ' . $value['title'] : '' );
				$results[] = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( ! is_wp_error( $product ) ) {
				...
				if ( ! empty( $product_title ) ) {
					$product_title_display = ' - ' . esc_html__( 'Title', 'frappe' ) . ': ' . $product_title;
				}
				
				$product_id_display = esc_html__( 'Id', 'frappe' ) . ': ' . $product_id;
