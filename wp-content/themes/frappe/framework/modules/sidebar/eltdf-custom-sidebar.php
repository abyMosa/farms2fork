 ...
 */
if ( ! class_exists( 'FrappeElatedClassSidebar' ) ) {
	class FrappeElatedClassSidebar {
		var $sidebars = array();
		var $stored = "";
		...
		function __construct() {
			...
			$this->title  = esc_html__( 'Custom Widget Area', 'frappe' );
		...
		function load_assets() {
			...
			wp_enqueue_script( 'frappe-elated-sidebar', ELATED_ROOT . '/framework/admin/assets/js/eltdf-ui/eltdf-sidebar.js' );
			wp_enqueue_style( 'frappe-elated-sidebar', ELATED_ROOT . '/framework/admin/assets/css/eltdf-sidebar.css' );
		}
		...
		function template_add_widget_field() {
			...
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '" . esc_attr__('Enter Name of the new Widget Area', 'frappe') . "' name='eltdf-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='" . esc_attr__('Add Widget Area', 'frappe') . "' />";
			echo "\n    " . $nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function register_custom_sidebars() {
			...
			$args = apply_filters( 'frappe_elated_filter_custom_widget_args', $args );
