...
namespace FrappeElatedNamespace\Modules\Header\Lib;
/**
 * Class FrappeElatedClassHeaderConnector
 *
...
class FrappeElatedClassHeaderConnector {
	...
	public function connect( $config = array() ) {
		do_action( 'frappe_elated_action_pre_header_connect' );
		...
		if ( ! empty( $config['affect_content'] ) ) {
			add_filter( 'frappe_elated_filter_content_elem_style_attr', array( $this, 'contentMarginFilter' ) );
		}
		
		if ( ! empty( $config['affect_title'] ) ) {
			add_filter( 'frappe_elated_filter_title_content_padding', array( $this, 'titlePaddingFilter' ) );
		}
		
		do_action( 'frappe_elated_action_after_header_connect' );
	}
	 ...
	 *
	 * Hooks to frappe_elated_filter_title_content_padding filter
	 *
	 * @return int
	 */
