...
if ( ! function_exists( 'frappe_elated_get_hide_dep_for_header_logo_area_meta_boxes' ) ) {
	function frappe_elated_get_hide_dep_for_header_logo_area_meta_boxes() {
		$hide_dep_options = apply_filters( 'frappe_elated_filter_header_logo_area_hide_meta_boxes', $hide_dep_options = array() );
		
		return $hide_dep_options;
	}
}

if ( ! function_exists( 'frappe_elated_get_hide_dep_for_header_logo_area_widgets_meta_boxes' ) ) {
	function frappe_elated_get_hide_dep_for_header_logo_area_widgets_meta_boxes() {
		$hide_dep_options = apply_filters( 'frappe_elated_filter_header_logo_area_widgets_hide_meta_boxes', $hide_dep_options = array() );
		
		return $hide_dep_options;
	}
}

if ( ! function_exists( 'frappe_elated_header_logo_area_meta_options_map' ) ) {
	function frappe_elated_header_logo_area_meta_options_map( $header_meta_box ) {
		$hide_dep_options = frappe_elated_get_hide_dep_for_header_logo_area_meta_boxes();
		$hide_dep_widgets = frappe_elated_get_hide_dep_for_header_logo_area_widgets_meta_boxes();
		...
		frappe_elated_add_admin_section_title(
			array(
				...
				'title'  => esc_html__( 'Logo Area Style', 'frappe' )
			)
		);
		...
		frappe_elated_create_meta_box_field(
			array(
				...
				'type'          => 'yesno',
				'default_value' => 'no',
				'label'         => esc_html__( 'Disable Header Logo Area Widget', 'frappe' ),
				'description'   => esc_html__( 'Enabling this option will hide widget area from the logo area', 'frappe' ),
				'parent'        => $logo_area_container,
				'dependency' => array(
					'hide' => array(
		...
		if ( count( $frappe_custom_sidebars ) > 0 ) {
			frappe_elated_create_meta_box_field(
				array(
					...
					'type'        => 'selectblank',
					'label'       => esc_html__( 'Choose Custom Widget Area for Logo Area', 'frappe' ),
					'description' => esc_html__( 'Choose custom widget area to display in header logo area"', 'frappe' ),
					'parent'      => $logo_area_container,
		...
		frappe_elated_create_meta_box_field(
			array(
				...
				'description' => esc_html__( 'Set grid background transparency for logo area (0 = fully transparent, 1 = opaque)', 'frappe' ),
				'parent'      => $logo_area_in_grid_container,
				'args'        => array(
					'col_width' => 2
				)
		...
		frappe_elated_create_meta_box_field(
			array(
				...
				'description'   => esc_html__( 'Set border on grid logo area', 'frappe' ),
				'parent'        => $logo_area_in_grid_container,
				'default_value' => '',
				'options'       => frappe_elated_get_yes_no_select_array()
			)
		...
		frappe_elated_create_meta_box_field(
			array(
				...
				'description' => esc_html__( 'Choose a background color for logo area', 'frappe' ),
				'parent'      => $logo_area_container
			)
		);
		...
		frappe_elated_create_meta_box_field(
			array(
				...
				'description' => esc_html__( 'Choose a transparency for the logo area background color (0 = fully transparent, 1 = opaque)', 'frappe' ),
				'parent'      => $logo_area_container,
				'args'        => array(
					'col_width' => 2
				)
		...
		$logo_area_border_bottom_color_container = frappe_elated_add_admin_container(
			array(
				'type'            => 'container',
		...
		do_action( 'frappe_elated_action_header_logo_area_additional_meta_boxes_map', $logo_area_container );
	}
	
	add_action( 'frappe_elated_action_header_logo_area_meta_boxes_map', 'frappe_elated_header_logo_area_meta_options_map', 10, 1 );
}