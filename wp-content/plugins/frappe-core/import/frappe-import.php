...
class FrappeCoreImport {
	...
	public function import_content( $file ) {
		ob_start();
		require_once( FRAPPE_CORE_ABS_PATH . '/import/class.wordpress-importer.php' );
		$frappe_import = new WP_Import();
		set_time_limit( 0 );
		
		$frappe_import->fetch_attachments = $this->attachments;
		$returned_value                  = $frappe_import->import( $file );
		
		if ( is_wp_error( $returned_value ) ) {
			$this->message = esc_html__( 'An Error Occurred During Import', 'frappe-core' );
		} else {
			$this->message = esc_html__( 'Content imported successfully', 'frappe-core' );
		}
	...
	public function import_widgets( $file, $file2 ) {
		...
		foreach ( (array) $options['widgets'] as $frappe_widget_id => $frappe_widget_data ) {
			update_option( 'widget_' . $frappe_widget_id, $frappe_widget_data );
		}
		
		$this->import_sidebars_widgets( $file );
		$this->message = esc_html__( 'Widgets imported successfully', 'frappe-core' );
	}
	
	public function import_sidebars_widgets( $file ) {
		$frappe_sidebars = get_option( "sidebars_widgets" );
		unset( $frappe_sidebars['array_version'] );
		$data = $this->file_options( $file );
		
		if ( is_array( $data['sidebars'] ) ) {
			$frappe_sidebars = array_merge( (array) $frappe_sidebars, (array) $data['sidebars'] );
			unset( $frappe_sidebars['wp_inactive_widgets'] );
			$frappe_sidebars                  = array_merge( array( 'wp_inactive_widgets' => array() ), $frappe_sidebars );
			$frappe_sidebars['array_version'] = 2;
			wp_set_sidebars_widgets( $frappe_sidebars );
		}
	}
	...
	public function import_custom_sidebars( $file ) {
		...
		$this->message = esc_html__( 'Custom sidebars imported successfully', 'frappe-core' );
	}
	
	public function import_options( $file ) {
		$options       = $this->file_options( $file );
		$result        = update_option( 'eltdf_options_frappe', $options );
		$this->message = esc_html__( 'Options imported successfully', 'frappe-core' );
	}
	
	public function import_menus( $file ) {
		global $wpdb;
		$frappe_terms_table = $wpdb->prefix . "terms";
		$this->menus_data  = $this->file_options( $file );
		$menu_array        = array();
		
		foreach ( $this->menus_data as $registered_menu => $menu_slug ) {
			$term_rows = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $frappe_terms_table where slug=%s", $menu_slug ), ARRAY_A );
	...
	public function import_settings_pages( $file ) {
		...
		foreach ( $pages as $frappe_page_option => $frappe_page_id ) {
			update_option( $frappe_page_option, $frappe_page_id );
		}
	}
	...
	function eltdf_admin_import() {
		if ( frappe_core_theme_installed() ) {
			...
			$this->pagehook = add_submenu_page(
                ELATED_OPTIONS_SLUG,
				esc_html__( 'Frappe Options - Frappe Import', 'frappe-core' ), // The value used to populate the browser's title bar when the menu page is active
				esc_html__( 'Import', 'frappe-core' ),                     // The text of the menu in the administrator's sidebar
				'administrator',                                          // What roles are able to access the menu
                ELATED_OPTIONS_SLUG . $slug,                  // The ID used to bind submenu items to this menu
				array( $frappe_elated_Framework->getSkin(), 'renderImport' )
			);
			
			add_action( 'admin_print_scripts-' . $this->pagehook, 'frappe_elated_enqueue_admin_scripts' );
			add_action( 'admin_print_styles-' . $this->pagehook, 'frappe_elated_enqueue_admin_styles' );
		}
	}
	...
	function eltdf_update_options_after_import( $folder ) {
		...
		foreach ( $demo_urls as $demo_url ) {
			$global_options    = get_option( 'eltdf_options_frappe' );
			$new_global_values = str_replace( $demo_url, $url, $global_options );

			update_option( 'eltdf_options_frappe', $new_global_values );
		}
	}
