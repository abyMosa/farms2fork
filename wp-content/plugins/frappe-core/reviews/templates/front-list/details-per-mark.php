<div class="eltdf-reviews-list-info eltdf-reviews-per-mark">
    <?php foreach($post_ratings as $rating) { ?>
        <?php
            $average_rating = frappe_core_post_average_rating($rating);
            $rating_count = $rating['count'];
            $rating_count_label = $rating_count === 1 ? esc_html__('Rating', 'frappe-core') : esc_html__('Ratings', 'frappe-core');
                ...
                <div class="eltdf-reviews-number-wrapper">
                    ...
                    <span class="eltdf-stars-wrapper">
                        ...
                        <span class="eltdf-reviews-count">
                            <?php echo esc_html__( 'Rated', 'frappe-core' ) . ' ' . $average_rating . ' ' . esc_html__( 'out of', 'frappe-core' ) . ' ' . $rating_count . ' ' . $rating_count_label; ?>
                        </span>
                    </span>
                </div>
            </div>
            <div class="eltdf-grid-col-8">
                <div class="eltdf-rating-percentage-wrapper">
                    ...
                    foreach ( $rating_marks as $item => $value ) {
                        ...
                        echo do_shortcode( '[eltdf_progress_bar percent="' . $percentage . '" title="' . $item . esc_attr__( ' stars', 'frappe-core' ) . '"]' );
                    }
                    ?>
                </div>
            </div>
