...
class ItemShowcase implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'      => esc_html__( 'Item Showcase', 'frappe-core' ),
					'base'      => $this->base,
					...
					'params'    => array(
						...
						array(
							...
							'heading'     => esc_html__( 'Image Top Offset', 'frappe-core' ),
							'value'       => '-100px',
							'save_always' => true
						)
					)
	...
	public function render( $atts, $content = null ) {
		...
		if ( ! empty( $image_top_offset ) ) {
			$item_image_style = 'margin-top: ' . frappe_elated_filter_px( $image_top_offset ) . 'px';
		}
		
		$html = '<div class="eltdf-item-showcase-holder clearfix">';
			$html .= do_shortcode( $content );
			if ( ! empty( $item_image ) ) {
				$html .= '<div class="eltdf-is-image" ' . frappe_elated_get_inline_style( $item_image_style ) . '>';
