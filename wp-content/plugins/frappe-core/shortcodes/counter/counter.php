<?php
namespace FrappeCore\CPT\Shortcodes\Counter;
...
class Counter implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Counter', 'frappe-core' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						...
						array(
							...
							'heading'     => esc_html__( 'Counter Custom Image', 'frappe-core' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'type',
							...
							'value'       => array(
								esc_html__( 'Zero Counter', 'frappe-core' )   => 'eltdf-zero-counter',
								esc_html__( 'Random Counter', 'frappe-core' ) => 'eltdf-random-counter'
							),
							'save_always' => true,
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'digit',
							'heading'    => esc_html__( 'Digit', 'frappe-core' )
						),
						array(
							'type'       => 'textfield',
						...
						array(
							...
							'heading'    => esc_html__( 'Digit Color', 'frappe-core' ),
							'dependency' => array( 'element' => 'digit', 'not_empty' => true )
						),
						array(
							'type'       => 'textfield',
						...
						array(
							...
							'heading'     => esc_html__( 'Title Font Weight', 'frappe-core' ),
	...
	public function render( $atts, $content = null ) {
		...
		$params['counter_image']  = $this->getCounterImage( $params );
		
		$params['title_tag'] = ! empty( $params['title_tag'] ) ? $params['title_tag'] : $args['title_tag'];
		
		$html = frappe_core_get_shortcode_module_template_part( 'templates/counter', 'counter', '', $params );
	...
	private function getCounterStyles( $params ) {
		...
		if ( ! empty( $params['digit_font_size'] ) ) {
			$styles[] = 'font-size: ' . frappe_elated_filter_px( $params['digit_font_size'] ) . 'px';
		}
	...
	private function getCounterImage($params) {
		$counter_image = '';

		$counter_image = wp_get_attachment_url( $params['counter_image'] );
