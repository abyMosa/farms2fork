<?php
namespace FrappeCore\CPT\Shortcodes\Banner;
...
class Banner implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Banner', 'frappe-core' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						...
						array(
							...
							'heading'    => esc_html__( 'Image Overlay Color', 'frappe-core' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'hover_behavior',
							'heading'     => esc_html__( 'Hover Behavior', 'frappe-core' ),
							'value'       => array(
								esc_html__( 'Visible on Hover', 'frappe-core' )   => 'eltdf-visible-on-hover',
								esc_html__( 'Visible on Default', 'frappe-core' ) => 'eltdf-visible-on-default',
								esc_html__( 'Disabled', 'frappe-core' )           => 'eltdf-disabled'
							),
							'save_always' => true
						),
						array(
							...
							'heading'     => esc_html__( 'Info Position', 'frappe-core' ),
							'value'       => array(
								esc_html__( 'Default', 'frappe-core' )  => 'default',
								esc_html__( 'Centered', 'frappe-core' ) => 'centered'
							),
							'save_always' => true
						),
						array(
							...
							'heading'     => esc_html__( 'Info Content Padding', 'frappe-core' ),
							'description' => esc_html__( 'Please insert padding in format top right bottom left', 'frappe-core' )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'subtitle',
						...
						array(
							...
							'heading'     => esc_html__( 'Subtitle Tag', 'frappe-core' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Subtitle Color', 'frappe-core' ),
							'dependency' => array( 'element' => 'subtitle', 'not_empty' => true )
						),
						array(
							'type'       => 'textfield',
						...
						array(
							...
							'heading'    => esc_html__( 'Link Text', 'frappe-core' ),
							'dependency' => array( 'element' => 'link', 'not_empty' => true )
						),
						array(
							'type'       => 'colorpicker',
							'param_name' => 'link_color',
							'heading'    => esc_html__( 'Link Text Color', 'frappe-core' ),
							'dependency' => array( 'element' => 'link', 'not_empty' => true )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'link_top_margin',
							'heading'    => esc_html__( 'Link Text Top Margin (px)', 'frappe-core' ),
							'dependency' => array( 'element' => 'link', 'not_empty' => true )
						)
					)
				)
	...
	public function render( $atts, $content = null ) {
		...
		$html = frappe_core_get_shortcode_module_template_part( 'templates/banner', 'banner', '', $params );
	...
	private function getLinkStyles( $params ) {
		...
		if ( ! empty( $params['link_top_margin'] ) ) {
			$styles[] = 'margin-top: ' . frappe_elated_filter_px( $params['link_top_margin'] ) . 'px';
		}
