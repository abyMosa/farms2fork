<?php
namespace FrappeCore\CPT\Shortcodes\Icon;
...
class Icon implements Lib\ShortcodeInterface {
    ...
    public function vcMap() {
	    if(function_exists('vc_map')) {
		    vc_map(
		    	array(
				    'name'                      => esc_html__( 'Icon', 'frappe-core' ),
				    'base'                      => $this->base,
				    ...
				    'params'                    => array_merge(
					    ...
					    \FrappeElatedClassIconCollections::get_instance()->getVCParamsArray(),
					    array(
						    array(
							    'type'       => 'dropdown',
							    'param_name' => 'size',
							    ...
							    'value'      => array(
								    ...
								    esc_html__( 'Huge', 'frappe-core' )   => 'eltdf-icon-huge'
							    )
						    ),
						    array(
							    'type'       => 'textfield',
							    'param_name' => 'custom_size',
							    'heading'    => esc_html__( 'Custom Size (px)', 'frappe-core' )
						    ),
						    array(
							    'type'       => 'dropdown',
							    'param_name' => 'type',
						    ...
						    array(
							    ...
							    'heading'     => esc_html__( 'Border Radius', 'frappe-core' ),
							    'description' => esc_html__( 'Please insert border radius(Rounded corners) in px. For example: 4 ', 'frappe-core' ),
						    ...
						    array(
							    ...
							    'heading'    => esc_html__( 'Border Width', 'frappe-core' ),
							    'dependency' => array( 'element' => 'type', 'value'   => array( 'eltdf-circle', 'eltdf-square' ) )
						    ),
						    array(
							    'type'       => 'colorpicker',
						    ...
						    array(
							    ...
							    'heading'    => esc_html__( 'Hover Icon Color', 'frappe-core' )
						    ),
						    array(
							    'type'       => 'colorpicker',
						    ...
						    array(
							    ...
							    'heading'     => esc_html__( 'Use Link as Anchor', 'frappe-core' ),
							    'value'       => array( 'Use this icon as Anchor?' => 'yes' ),
							    'description' => esc_html__( 'Check this box to use icon as anchor link (eg. #contact)', 'frappe-core' ),
	...
	public function render( $atts, $content = null ) {
		...
		$html = frappe_core_get_shortcode_module_template_part( 'templates/icon', 'icon', '', $params );
	...
	private function generateIconStyles( $params ) {
		...
		if ( ( $params['type'] !== 'eltdf-normal' && ! empty( $params['shape_size'] ) ) || ( $params['type'] == 'eltdf-normal' ) ) {
			if ( ! empty( $params['custom_size'] ) ) {
				$iconStyles[] = 'font-size:' . frappe_elated_filter_px( $params['custom_size'] ) . 'px';
			}
		}
	...
	private function generateIconHolderStyles( $params ) {
		...
		if ( $params['type'] !== 'eltdf-normal' ) {
			...
			if ( ! empty( $shapeSize ) ) {
				$iconHolderStyles[] = 'width: ' . frappe_elated_filter_px( $shapeSize ) . 'px';
				$iconHolderStyles[] = 'height: ' . frappe_elated_filter_px( $shapeSize ) . 'px';
				$iconHolderStyles[] = 'line-height: ' . frappe_elated_filter_px( $shapeSize ) . 'px';
			}
			...
			if ( ! empty( $params['border_color'] ) && ( isset( $params['border_width'] ) && $params['border_width'] !== '' ) ) {
				...
				$iconHolderStyles[] = 'border-width: ' . frappe_elated_filter_px( $params['border_width'] ) . 'px';
			...
			if ( $params['type'] == 'eltdf-square' ) {
				if ( isset( $params['border_radius'] ) && $params['border_radius'] !== '' ) {
					$iconHolderStyles[] = 'border-radius: ' . frappe_elated_filter_px( $params['border_radius'] ) . 'px';
				}
			}
		}
